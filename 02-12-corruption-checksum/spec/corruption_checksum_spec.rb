...
RSpec.describe CorruptionChecksum do
  ...
  sorted_lines = [[1, 5, 5, 9], [3, 5, 7], [2, 4, 6, 8]]
  values = [8, 4, 6]
  context "split lines" do
    it "splits strings into individual lines" do
      expect(checksum.get_values(input)).to eq sorted_lines
    end
  end

  context "finds total per line" do
    it "splits strings into individual lines" do
      expect(checksum.get_difference(sorted_lines)).to eq values
    end
  end

  context "sums the numbers" do
    it "sums the numbers" do
      expect(checksum.sum(values)). to eq 18
    end
  end

  context "all together" do
    it "spits out the value for an input" do
      expect(checksum.compute_sum(input)).to eq 18
    end
  end

  context "big one" do
    it "gives the value for a large input" do
      large_input = "116    1470    2610    179    2161    2690    831    1824    2361    1050    2201    118    145    2275    2625    2333
      ...
      expect(checksum.compute_sum(large_input)).to eq 32020
    end
  end
end
